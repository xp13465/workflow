(function(){
    var ctrl = $M.ctrl.extend({
        ...
        //发布公告
        __released_publicMessage__ : function(b){
            var me=this;
            var params = me.__getParams( b );
            $M.Message.confirm('提示','确认操作吗','submit',function(result){
                if(result==true){
                    $M.Ajaxs({
                        url:'/Admin/Message/releasedPublicMessage',
                        data:{id:params.id},
                    },function(data){
                        if(data.status>0){
                            $M.Message.alert('提示','操作成功','info');
        ...
        //还原公告
        __restore_publicMessage__ : function(b){
            var me=this;
            var params = me.__getParams( b );
            $M.Message.confirm('提示','确认操作吗','submit',function(result){
                if(result==true){
                    $M.Ajaxs({
                        url:'/Admin/Message/restoreSenderPublicMessage',
                        data:{id:params.id},
                    },function(data){
                        if(data.status>0){
                            $M.Message.alert('提示','操作成功','info');
