...
Ext.define('ui.ctrl.productmeeting.productmeetingCtrl', {
    extend: 'ui.extend.baseClass.baseCtrl',
    requires : [
        'ui.ctrl.productmeeting.productmeetingConf',
        'ui.view.productmeeting.productmeetingView',
        'ui.view.productmeeting.Coms.viewPanel'
    ],
    constructor : function (cfg){
        this.modelConf = Ext.create('ui.ctrl.productmeeting.productmeetingConf' , {ctrl : cfg.ctrl});
        this.callParent(arguments);
    },

    init : function(){
        this.control({      //这里的this相当于这个控制层
            'productmeetingpagecentergrid' : {
                render : function(e){
    ...
    //配置功能按钮功能  对应Conf类中定义的operationButtons > pmsCode { text : '新增角色+' , pmsCode : 'productmeeting.add' }
    //‘.’替换为 $  ,方法前加__
    __productmeeting$add : function( btna , params ){
        var me = this;
    ...
    __productmeeting$view : function( btna , params ){
        var me = this;
        var view = this.$getView();
        Ext.apply(params,{callback: function(panel,win){
            //me.$addWindowToFather(win);
    ...
    __productmeeting$edit : function( btna , params ){
        var me = this;
        var view = this.$getView();
        Ext.apply(params,{callback: function(panel,win){
            //me.$addWindowToFather(win);
    ...
    __productmeeting$submit : function(btna , params){
        var me = this;
    ...
    __productmeeting$unsubmit : function( btna , params ){
        var me = this;
        if(!btna.pmsCode){
    ...
    __productmeeting$cancel : function( btna , params ){
        var me = this;
        var pmsCode = btna.pmsCode?btna.pmsCode:($(btna).attr('pmscode')?$(btna).attr('pmscode'):null);
        //var records = me.$getGridSelections();
...
/*SUBSUBSUBSUB-----END*/
    ...
    __check_list_btn_event : function(cfg ,record){
        if(cfg.pmsCode === 'productmeeting.submit' || cfg.pmsCode === 'productmeeting.unsubmit'){
