...
Ext.define('ui.ctrl.addressbookout.addressbookoutCtrl', {
    extend: 'ui.extend.baseClass.baseCtrl',
    requires : [
        'ui.ctrl.addressbookout.addressbookoutConf',
        'ui.view.addressbookout.addressbookoutView',
        'ui.view.addressbookout.Coms.viewPanel'
    ],
    views : [
        'ui.view.addressbookout.addressbookoutView'
    ],
    ...
    refs:[
        {ref:'addressbookoutWin' , selector:'addressbookoutwindow'}
    ],

    constructor : function (cfg){
        this.modelConf = Ext.create('ui.ctrl.addressbookout.addressbookoutConf');
        this.callParent(arguments);
    },

    init:function(){
        this.control({      //这里的this相当于这个控制层
            'addressbookoutpagecentergrid' : {
                render : function(e){
    ...
    //配置功能按钮功能  对应Conf类中定义的operationButtons > pmsCode { text : '新增角色+' , pmsCode : 'addressbookout.add' }
    //‘.’替换为 $  ,方法前加__
    __addressbookout$add : function( btna , params ){
        var me = this;
    ...
    __addressbookout$view : function( btna , params ){
        var me = this;
        var pmsCode = btna.pmsCode?btna.pmsCode:($(btna).attr('pmscode')?$(btna).attr('pmscode'):null);
        if(!pmsCode) return;
        var param  = params || {};
    ...
    __addressbookout$edit : function( btna , params ){
        var me = this;
        var pmsCode = btna.pmsCode?btna.pmsCode:($(btna).attr('pmscode')?$(btna).attr('pmscode'):null);
        if(!pmsCode) return;
        var param  = params || {};
