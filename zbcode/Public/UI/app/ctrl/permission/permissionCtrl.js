...
Ext.define('ui.ctrl.permission.permissionCtrl', {
    extend: 'ui.extend.baseClass.baseCtrl',
    requires : [
        'ui.ctrl.permission.permissionConf',
        'ui.view.permission.permissionView',
        'ui.view.permission.Coms.viewPanel'
    ],
    views : [
        'ui.view.permission.permissionView'
    ],
    ...
    refs:[
        {ref:'permissionWin' , selector:'permissionwindow'}
    ],

    constructor : function (cfg){
        this.modelConf = Ext.create('ui.ctrl.permission.permissionConf');
        this.callParent(arguments);
    },

    init:function(){
        this.control({      //这里的this相当于这个控制层
            'permissionpagecentergrid' : {
                render : function(e){
    ...
    //配置功能按钮功能  对应Conf类中定义的operationButtons > pmsCode { text : '新增角色+' , pmsCode : 'permission.add' }
    //‘.’替换为 $  ,方法前加__
    __permission$add : function( btna , params ){
    ...
    __permission$view : function( btna , params ){
        var me = this;
        var pmsCode = btna.pmsCode?btna.pmsCode:($(btna).attr('pmscode')?$(btna).attr('pmscode'):null);
        if(!pmsCode) return;
        var param  = params || {};
    ...
    __permission$edit : function( btna , params ){
        var me = this;
        var pmsCode = btna.pmsCode?btna.pmsCode:($(btna).attr('pmscode')?$(btna).attr('pmscode'):null);
        if(!pmsCode) return;
        var param  = params || {};
    ...
    __permission$start : function( btna , params ){
        var me = this;
        ...
        me.$askYestNo({
            ...
            yes : function(){
                me.$requestAjax({
                    url     :   me.$getUrlstrByUrlcode('set.permission.change'),
                    method :    'POST',
                    params :    params,
                    scope  :    me,
                    backParams: {},
    ...
    __permission$stop : function( btna , params ){
        var me = this;
        params = me.$getArrayOne(params);
        if(params.status == 2){
            Ext.Msg.show({
    ...
    __permission$delete : function( btna , params ){
        var me = this;
        params = me.$getArrayOne(params);
        if(params.status == 1){
            Ext.Msg.show({
        ...
        me.$askYestNo({
            ...
            yes : function(){
                me.$requestAjax({
                    url     :   me.$getUrlstrByUrlcode('set.permission.delete'),
                    method :    'POST',
                    params :    params,
                    scope  :    me,
                    backParams: {},
    ...
    __permission$cexiao : function( btna , params ){
        var me = this;
        // params = me.$getArrayOne(params);
        var ids=[];
        ...
        Ext.each(records,function(item){
            ...
            if(item.data.status=='1'){
                ...
                ids.push(item.data.permission_id);
            }
            
        });
        postParams.permission_id=ids.join();
        postParams.status=3;
