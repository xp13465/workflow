...
Ext.define('ui.ctrl.workplatformleader.workplatformleaderCtrl', {
    extend: 'ui.extend.baseClass.baseCtrl',
    requires : [
        'ui.ctrl.workplatformleader.workplatformleaderConf',
        'ui.view.workplatformleader.workplatformleaderView',
        'ui.view.workplatformleader.Coms.viewPanel'
    ],
    views : [
        'ui.view.workplatformleader.workplatformleaderView'
    ],
    ...
    refs:[
        {ref:'workplatformleaderWin' , selector:'workplatformleaderwindow'}
    ],

    constructor : function (cfg){
        this.modelConf = Ext.create('ui.ctrl.workplatformleader.workplatformleaderConf' , {ctrl : cfg.ctrl});
        this.callParent(arguments);
    },

    init : function(){
        this.control({      //这里的this相当于这个控制层
            'workplatformleaderpagecentergrid' : {
                render : function(e){
    ...
    //配置功能按钮功能  对应Conf类中定义的operationButtons > pmsCode { text : '新增角色+' , pmsCode : 'workplatformleader.add' }
    //‘.’替换为 $  ,方法前加__
    __workplatformleader$add : function( btna , params ){
        var me = this;
        var view = this.$getView();
        Ext.apply(params,{ pmsCode : "workplatformleader"});
 ...
 /*
    ...
    __workplatformleader$view : function( btna , params ){
        var me = this;
        var view = this.$getView();
        Ext.apply(params,{callback: function(panel,win){
            //me.$addWindowToFather(win);
...
/*        var me = this;
    ...
    __workplatformleader$edit : function( btna , params ){
        var me = this;
        var view = this.$getView();
        Ext.apply(params,{callback: function(panel,win){
            //me.$addWindowToFather(win);
...
/*      var me = this;
    ...
    __workplatformleader$submit : function( btna , params ){
        var me = this;
        if(!btna.pmsCode){
    ...
    __workplatformleader$plcexiao : function(btna , params){
        var me = this;
        var records = me.$getGridSelections();
        me.__sub_cexiao_product.call(me,records);
    },

    __workplatformleader$cexiao : function(btna , params){
        var me = this;
        var pmsCode = btna.pmsCode?btna.pmsCode:($(btna).attr('pmscode')?$(btna).attr('pmscode'):null);
        var records = me.$getGridSelections();
        me.__sub_cexiao_product.call(me,records);
    ...
    */
    ...
    __all_main_render : function(thepanel){
        ...
        if(!theFeildPanel){
            Ext.require([
                ...
                theFeildPanel = Ext.create("ui.view.workplatformleader.Coms.extraFieldPanel");
    ...
    __all_post_after_main_event : function(thepanel){
        ...
        if(Ext.Array.indexOf(['view','add','edit'],optype)>-1){
            ...
            if(feeInfo && feeInfo.product_id>0){
...
/*SUBSUBSUBSUB-----END*/
    ...
    __check_list_btn_event : function(cfg ,record){
        if(cfg.pmsCode === 'workplatformleader.submit'){
            if(record.data.status >1 ) return false;
        }
        if(cfg.pmsCode === 'workplatformleader.edit'){
            if(record.data.status >1 ) return false;
        }
        if(cfg.pmsCode === 'workplatformleader.cexiao' || cfg.pmsCode === 'workplatformleader.plcexiao'){
            if(record.data.status != 1 ) return false;
        }
        return true;
    }
