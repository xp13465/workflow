...
Ext.define('ui.ctrl.producttypeparam.producttypeparamCtrl', {
    extend: 'ui.extend.baseClass.baseCtrl',
    requires : [
        'ui.ctrl.producttypeparam.producttypeparamConf',
        'ui.view.producttypeparam.producttypeparamView',
        'ui.view.producttypeparam.Coms.viewPanel'
    ],
    constructor : function (cfg){
        this.modelConf = Ext.create('ui.ctrl.producttypeparam.producttypeparamConf');
        this.callParent(arguments);
    },

    init:function(){
        this.control({      //这里的this相当于这个控制层
            'producttypeparampagecentergrid' : {
                render : function(e){
    ...
    //配置功能按钮功能  对应Conf类中定义的operationButtons > pmsCode { text : '新增角色+' , pmsCode : 'producttypeparam.add' }
    //‘.’替换为 $  ,方法前加__

    __producttypeparam$add : function( btna , params ){
        var me = this;
    ...
    __producttypeparam$view : function( btna , params ){
        var me = this;
        var pmsCode = btna.pmsCode?btna.pmsCode:($(btna).attr('pmscode')?$(btna).attr('pmscode'):null);
        if(!pmsCode) return;
        var param  = params || {};
    ...
    __producttypeparam$edit : function( btna , params ){
    ...
    __producttypeparam$start : function( btna , params ){
        var me = this;
        ...
        me.$askYestNo({
            ...
            yes : function(){
                me.$requestAjax({
                    url     :   me.$getUrlstrByUrlcode('set.producttypeparam.change'),
                    method :    'POST',
                    params :    params,
                    scope  :    me,
                    backParams: {},
    ...
    __producttypeparam$stop : function( btna , params ){
        var me = this;
        params = me.$getArrayOne(params);
        if(params.status == 2){
            Ext.Msg.show({
    ...
    __producttypeparam$delete : function( btna , params ){
        var me = this;
        params = me.$getArrayOne(params);
        if(params.status == 1){
            Ext.Msg.show({
        ...
        me.$askYestNo({
            ...
            yes : function(){
                me.$requestAjax({
                    url     :   me.$getUrlstrByUrlcode('set.producttypeparam.delete'),
                    method :    'POST',
                    params :    params,
                    scope  :    me,
                    backParams: {},
    ...
    __producttypeparam$cexiao : function( btna , params ){
        var me = this;
        // params = me.$getArrayOne(params);
        var ids=[];
    ...
    __add_category_field_by_data : function(params ,callback){
        var me = this;
        me.$requestAjax({
            url     :   me.$getUrlstrByUrlcode('get.producttypeparam.adCategoryField'),
            method :    'POST',
            params :    params,
            scope  :    me,
            backParams: {},
    ...
    */
    ...
    __all_post_after_main_event : function(thepanel){
        ...
        var panel = Ext.create("ui.view.producttypeparam.Coms.subTabPanel",{
            ctrl : me,
            title:'类型参数',
    ...
    __del_group_field_by_id : function(params ,callback){
        ...
        me.$askYestNo({
            ...
            yes : function(){
                me.$requestAjax({
                    url     :   me.$getUrlstrByUrlcode('get.producttypeparam.del'),
                    method :    'POST',
    ...
    __edit_category_field_by_id : function(params , callback){
        ...
        if(!params.id && !params.field_id) return false;
        me.$requestAjax({
            url     :   me.$getUrlstrByUrlcode('get.producttypeparam.editScoreField'),
            method :    'POST',
            params :    params,
            scope  :    me,
            backParams: {},
