...
Ext.define('ui.view.role.Coms.ListGrid',{
    extend: 'ui.extend.base.Panel',
    alias: 'widget.rolepagecentergrid',
    requires :[
        'ui.extend.base.Grid'
    ],
    constructor : function(cfg){
        ...
        this.ctrl = cfg.ctrl;
//定义数据源store
        var url = this.ctrl.$getConfig('urls').get('get.role.list').url;
        this.store = Ext.create( 'ui.store.role.roleStore' , {
            ctrl : this.ctrl,
            autoLoad: { params: {} },
            proxy : Ext.create( 'ui.extend.base.Ajax',{
                url : url     //"/Public/jsons/get.all.role.record.json",
            })
        });
//定义checkboxModel
...
//定义字段列表
        ...
        cmconfig = cmconfig || [];
        cmconfig.push({
            header: '操作', width: 180, dataIndex: 'oprations',
