...
Ext.define('ui.view.position.Coms.ListGrid',{
    extend: 'ui.extend.base.Panel',
    alias: 'widget.positionpagecentergrid',
    requires :[
        'ui.extend.base.Grid'
    ],
    constructor : function(cfg){
        ...
        this.ctrl = cfg.ctrl;
//定义数据源store
        var url = this.ctrl.$getConfig('urls').get('get.position.list').url;
        this.store = Ext.create( 'ui.store.positionStore' , {
            fields : [],
            ctrl : this.ctrl,
            autoLoad: { params: {} },
            proxy : Ext.create( 'ui.extend.base.Ajax',{
                url : url     //"/Public/jsons/get.all.position.record.json",
            })
        });
//定义checkboxModel
