...
Ext.define('ui.view.addressbook.Coms.ListGrid',{
    extend: 'ui.extend.base.Panel',
    alias: 'widget.addressbookpagecentergrid',
    requires :[
        'ui.extend.base.Grid'
    ],
    constructor : function(cfg){
        ...
        this.ctrl = cfg.ctrl;
//定义数据源store
        var url = this.ctrl.$getConfig('urls').get('get.addressbook.list').url;
        this.store = Ext.create( 'ui.store.userStore' , {
            fields : [],
            ctrl : this.ctrl,
            autoLoad: { params: {} },
            proxy : Ext.create( 'ui.extend.base.Ajax',{
                url : url     //"/Public/jsons/get.all.addressbook.record.json",
            })
        });
//定义checkboxModel
...
//定义gridpanel
        this.grid = Ext.create('ui.extend.base.Grid',{
            myid : 'ListGrid',
            alias: ["widget.addressbooklistgrid"],
            selModel: sm,
            tbar : this.form,
            border : 0,
            store: this.store,
