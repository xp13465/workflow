...
Ext.define('ui.view.productaudit.Coms.ListGrid',{
    extend: 'ui.extend.base.Panel',
    alias: 'widget.productauditpagecentergrid',
    requires :[
        'ui.extend.base.Grid'
    ],
    constructor : function(cfg){
        ...
        this.ctrl = cfg.ctrl;
//定义数据源store
        var url = this.ctrl.$getConfig('urls').get('get.productaudit.list').url;
        this.store = Ext.create( 'ui.store.productStore' , {
            fields : [],
            ctrl : this.ctrl,
            autoLoad: { params: {} },
            proxy : Ext.create( 'ui.extend.base.Ajax',{
                url : url     //"/Public/jsons/get.all.productaudit.record.json",
            })
        });
//定义checkboxModel
...
//定义gridpanel
        this.grid = Ext.create('ui.extend.base.Grid',{
            myid : 'ListGrid',
            alias: ["widget.productauditlistgrid"],
            selModel: sm,
            tbar : this.form,
            border : 0,
            store: this.store,
