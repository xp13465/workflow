...
Ext.define('ui.view.productleadermeeting.Coms.ListGrid',{
    extend: 'ui.extend.base.Panel',
    alias: 'widget.productleadermeetingpagecentergrid',
    requires :[
        'ui.extend.base.Grid'
    ],
    constructor : function(cfg){
        ...
        this.ctrl = cfg.ctrl;
//定义数据源store
        var url = this.ctrl.$getConfig('urls').get('get.productleadermeeting.list').url;
        this.store = Ext.create( 'ui.store.productStore' , {
            fields : [],
            ctrl : this.ctrl,
            autoLoad: { params: {} },
            proxy : Ext.create( 'ui.extend.base.Ajax',{
                url : url     //"/Public/jsons/get.all.productleadermeeting.record.json",
            })
        });
//定义checkboxModel
...
//定义字段列表
        ...
        cmconfig = cmconfig || [];
        cmconfig.push({
            header: '操作', width: 200, dataIndex: 'oprations',
...
//定义gridpanel
        this.grid = Ext.create('ui.extend.base.Grid',{
            myid : 'ListGrid',
            alias: ["widget.productleadermeetinglistgrid"],
            selModel: sm,
            tbar : this.form,
            border : 0,
            store: this.store,
